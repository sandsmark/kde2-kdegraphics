...
void DrawView::savePic()
  ...
  filename = KFileDialog::getSaveFileName( QString::null, KImageIO::pattern(KImageIO::Writing), this );
  if ( !filename.isNull() )
    {
    ...
    bitBlt( buffer, 0, 0, 
            ...
            CopyROP, FALSE );
    buffer->save( filename, KImageIO::type(filename).ascii() );
    delete buffer;
    }
  }
...
void DrawView::loadParam( const QString & name )
  ...
  if ( name == NULL )
    {
    filename = KFileDialog::getOpenFileName ( QString::null, "*.fct", this );
    if ( filename.isNull() )
      {
      return;
      }
