...
static const int TIMER_INTERVALL = 500;
static const int CURSOR_WIDTH = 9; // has to be odd number
static const int CURSOR_HEIGHT = 15;
...
Text::Text()
  : Tool(),
    ...
    charBuffer(QString::null)
{
  lineStack.setAutoDelete(true);  
}

Text::Text(const QString & toolname)
  : Tool(toolname),
    startx(0), // starty(0),
    cursorH(CURSOR_HEIGHT),
    lastx(0), lasty(0),
...
Text::mouseReleaseEvent(QMouseEvent *e)
{
  if (abs(e->x()-lastx)<3 && abs(e->y()-lasty)<3) {
    ...
    startx = lastx = e->x();
    starty = lasty = e->y();
    //    kdDebug() << "linestack count " << lineStack.count() << endl;
    // clear stack
    while ((i=lineStack.pop()) != NULL)
      delete i;
    //    kdDebug() << "linestack count " << lineStack.count() << endl;
    // here paint a blinking textmarker
    //    kdDebug() << "text enabled" << endl;
...
Text::keyPressEvent(QKeyEvent *e)
{
  ...
  if (Key_Backspace == key || Key_Delete == key ) { // one char back
    ...
    kdDebug() << "remove " << (char)lastc << endl;
    if (lastc == '\n') {
      ...
      QRect r = paint.fontMetrics().boundingRect(QChar(lastc));
      QString str (lastc);
      lastx -= w;
  ...
  QRect r = paint.fontMetrics().boundingRect(QChar(str[0]));
  // set cursor one left
  if (showingC) {
    paintC(&paint);
    lastx += w;
...
QPixmap Text::pixmap() const
{
    return UserIcon("largetext");
}
