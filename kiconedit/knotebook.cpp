...
KNoteBook::~KNoteBook()
{
  //kdDebug() << "KNoteBook - destructor" << endl;
  delete pnote;
  delete sections;
  //kdDebug() << "KNoteBook - destructor done" << endl;
}

void KNoteBook::init()
{
  //kdDebug() << "KNoteBook::init" << endl;
...
{
  //kdDebug() << "Activated: " << c << endl;
  pnote->currentmenu = c;
}

void KNoteBook::tabScroll( ArrowType )
{
  //kdDebug() << "KNoteBook::tabScroll" << endl;
...
{
  //kdDebug() << "ActivatedRedirect: " << c << endl;
...
int KNoteBook::addPage(KWizardPage *p)
{
  if(!pnote->numtabs)
  {
    kdDebug() << "Trying to add page when no KWizards are added!" << endl;
    return -1;
  }
...
QSize KNoteBook::childSize()
{
  ...
  for(int i = 0; i < pnote->numtabs; i++)
  {
    ...
    //kdDebug() << "Child size: " << size.width() << " x " << size.height() << endl;
  }
...
void KNoteBook::directionButton(bool changetab, bool forward)
{
  ...
  if(changetab)
  {
    if(forward)
    {
      //kdDebug() << "changing to tab: " << current+1 << endl;
      ...
      //kdDebug() << "Setting previous to: " << str << endl;
      button->show();
    ...
    {
      //kdDebug() << "changing to tab: " << pnote->current-1 << endl;
      gotoTab(pnote->current-1);
      ...
      //kdDebug() << "Setting next to: " << str << endl;
      button->show();
      if(pnote->enablearrowbuttons)
        arrow->show();
    }
...
void KNoteBook::setSizes()
{
  ...
  setMinimumSize(x, y);
  resize(x, y);
  //kdDebug() << "setSizes - done" << endl;
}

void KNoteBook::resizeEvent(QResizeEvent *)
{
  //kdDebug() << "KNote, resizing" << endl;
  ...
  //kdDebug() << "KNote, resize - done" << endl;
}

void KNoteBook::paintEvent(QPaintEvent *)
{
  //kdDebug() << "KNoteBook::paintEvent" << endl;
  // Check to see if there are any buttons.
  QPushButton *tmp = 0L;
