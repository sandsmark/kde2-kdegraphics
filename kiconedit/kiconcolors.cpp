...
{
  kdDebug() << "KDrawColors - constructor" << endl;
  setCellSize(17);
  setGrid(true);
  setGridState(KColorGrid::Shaded);
  selected = 0;
  kdDebug() << "KDrawColors - constructor - done" << endl;
}
...
{
  kdDebug() << "KSysColors - constructor" << endl;
  ...
  kdDebug() << "KSysColors - constructor - before setColor" << endl;
  for(int i = 0; i < numcells; i++)
    setColor(i, iconpalette[i]|OPAQUE_MASK);
  kdDebug() << "KSysColors - constructor - done" << endl;
}

KCustomColors::KCustomColors(QWidget *parent) : KDrawColors(parent)
{
  kdDebug() << "KCustomColors - constructor" << endl;
  setNumRows(3);
  setNumCols(6);
  ...
  for(int i = 0; i < numRows()*numCols(); i++)
    freecells[i] = true;
  kdDebug() << "KCustomColors - constructor - done" << endl;
}
...
void KCustomColors::addColor(uint c)
{
  if(!contains(c))
  {
    ...
    if(f != -1)
    {
      ...
      if(!color.isValid())
      {
        kdDebug() << "KCustomColors::addColor: Not a valid color: " << c << endl;
        return;
      }
      //kdDebug() << "KCustomColors::addColor: Adding color: " << c << " - " << qRed(c) << " " << qGreen(c) << " " << qBlue(c) << endl;
      setColor(f, c);
      freecells[f] = false;
    }
  }
