...
void setupImageHandlers()
{
  ...
  kdDebug() << "Initializing formats" << endl;
  ...
  formats->append(new imageFormat("ICO", "Icon File",  "ico"));
/*
#ifdef HAVE_LIBJPEG
...
bool copyFile(const QString &src, const QString &dest)
{
  ...
  kdDebug() << "Size: " << src_size << endl;

  if ( f_src.open(IO_ReadOnly) )
  {    // file opened successfully      
    if ( !f_dest.open(IO_WriteOnly) )
    {
      kdDebug() << "copyFile - There was an error opening destination file: " << dest << endl;
      f_src.close();
      return false;
    }
    ...
    {
      kdDebug() << "copyFile - There was an error reading source file: " << src << endl;
      f_src.close();
      f_dest.close();
      delete [] data;
      return false;
    ...
    {
      kdDebug() << "copyFile - There was an error writing to destination file: " << dest << endl;
      f_src.close();
      f_dest.close();
      delete [] data;
      return false;
  ...
  kdDebug() << "copyFile - There was an error opening source file: " << src << endl;
  return false;
}
...
bool removeFile(const QString &file)
{
  if(file.length() > 0 && QFile::exists(file))
  {
    ...
    if(!d.remove(file))
    {
      kdDebug() << "removeFile - There was an error removing the file: " << file << endl;
      return false;
    }
    return true;
  }
