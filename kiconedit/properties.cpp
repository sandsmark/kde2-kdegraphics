...
{
    instances++;
    //kdDebug() << "KIconEditProperties: instances " << instances << endl;
    if(pprops) return;
    ...
    pprops->recentlist = new QStringList;
    ...
    *pprops->recentlist = config->readListEntry("RecentOpen");
    int n = pprops->recentlist->count();
  
    kdDebug() << "Read " << n << " recent files" << endl;
...
KIconEditProperties::~KIconEditProperties()
{
    ...
    if(instances == 0)
    {
        ...
        delete pprops;
        kdDebug() << "KIconEditProperties: Deleted properties" << endl;
    }
}
...
void KIconEditProperties::saveProperties(QWidget *parent)
{
    ...
    config->setGroup( "Files" );
    kdDebug() << "Writing " << pprops->recentlist->count() << " recent files" << endl;
